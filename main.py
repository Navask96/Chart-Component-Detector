...
def main(img):
    ...
    det_boxes, det_texts = detect(img)
    ...
    # making a copy of the image, plotting bboxes on image and removing detected bounding boxes
        ...
        print("No components detected. Check your object detection model.")
    ...
    h = len(comp_dim_tmp) - 2*det_boxes.shape[0]
    ...
    maps,node_comp_map,node_node_map = mapping(det_boxes,comp_dim,nodes)
    ...
    comp_list = []
    for i in range(det_boxes.shape[0]):
        # Add these print statements to your code for debugging
        print("Size of det_boxes:", det_boxes.shape)
        print("Index i:", i)
        print("Value at det_boxes[i]:", det_boxes[i])
        print("detText", det_texts[i])

        cl = int(det_boxes[i][5])
        dim = det_boxes[i]
        start = (int(dim[0]),int(dim[1]))
        end = (int(dim[2]),int(dim[3]))
    ...
    f.close()
    return result, boxes1, main_img1, comp_list, jns_list, conn_list, det_texts